...
exports.reg = async (ctx, next) => {
    ...
    try {
        const user = await mysqlModel.findUser(body.user_id)
        console.log(user)
        if (user) {
            ctx.body = ApiErrorNames.getErrorInfo(ApiErrorNames.USER_HAS_EXISTED)
            return
        }else{
            let {user_id, user_name, user_pwd} = body
            console.log(user_id, user_name, user_pwd)
            await mysqlModel.insertUserinfo([user_id, user_name, user_pwd])
...
exports.login = async (ctx, next) => {
    ...
    try {
        ...
        // 匹配密码是否相等
        if ((await user.user_pwd) === bodys.user_pwd) {
            let data = {
                user: user.user_id,
                // 生成 token 返回给客户端
                token: jwt.sign(
